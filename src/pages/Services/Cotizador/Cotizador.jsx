...
export default function Cotizador() {
    ...
    const { cart, addItem, removeItem, total, checkout } = useCartWithFirestore(user);
    ...
    const filteredPackages = packages.filter((p) => category === "all" || p.category === category);
    ...
    return (
        <div className="min-h-screen bg-[#281e76] p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-6 gap-6 mt-6">
                <Cart cart={cart} removeItem={removeItem} total={total} checkout={checkout} user={user} login={login} />
                    ...
                    {(type === "all" || type === "services") && (
                        <section>
                            ...
                            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                {services.map((s) => <ServiceCard key={s.id} service={s} onAdd={addItem} />)}
                            </div>
                        </section>
                    )}
                    ...
                    {(type === "all" || type === "packages") && (
                        <section>
                            ...
                            <div className="grid sm:grid-cols-2 gap-4">
                                {filteredPackages.map((p) => <PackageCard key={p.id} pack={p} onAdd={addItem} />)}
                            </div>
                        </section>
                    )}
                </div>
